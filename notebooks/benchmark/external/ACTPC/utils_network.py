...
from tensorflow.contrib.layers import fully_connected as FC_Net  # pyright: ignore
    ...
    for _ in range(num_layers):
        ...
        if keep_prob is not None:
            cell = tf.contrib.rnn.DropoutWrapper(
                cell, input_keep_prob=keep_prob, output_keep_prob=keep_prob  # pyright: ignore
            )  # state_keep_prob=keep_prob
        cells.append(cell)  # pyright: ignore
...
def create_concat_state_h(state, num_layers, RNN_type, BiRNN=None):
    ...
    for i in range(num_layers):
        ...
        if i == 0:
            rnn_state_out = tmp  # pyright: ignore
        else:
            rnn_state_out = tf.concat([rnn_state_out, tmp], axis=1)  # pyright: ignore

    return rnn_state_out  # pyright: ignore
...
def create_FCNet(inputs, num_layers, h_dim, h_fn, o_dim, o_fn, w_init, w_reg=None, keep_prob=1.0):
    ...
    w_init           : initialization for weight matrix (default: Xavier)
    keep_prob        : keep probability [0, 1]  (if None, dropout is not employed)
    """
