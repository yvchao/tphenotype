...
class DeepTPC_ICLR:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            """
            inputs = self.x
            inputs_ta = tf.TensorArray(dtype=tf.float32, size=self.max_length).unstack(
            ...
            mean_pis = tf.reduce_sum(tf.reduce_sum(tmp_pis, axis=1), axis=0) / tf.reduce_sum(
            ...
            for i in range(self.K):
                for j in range(i + 1, self.K):
                    self.LOSS_Ey += -loss_1(Ey[i, :], Ey[j, :], y_type_=self.y_type) / (
                        (self.K) * (self.K - 1)
    ...
    def predict_zs_and_pis_m2(self, x_):
        return self.sess.run(
            [self.zs, self.pis, self.rnn_mask2],
    ...
    def predict_zbars_and_pis_m1(self, x_):
        return self.sess.run(
            [self.z_bars, self.pis, self.rnn_mask1],
            feed_dict={self.x: x_, self.mb_size: np.shape(x_)[0], self.keep_prob: 1.0},
        )

    def predict_zs_and_pis_m1(self, x_):
        return self.sess.run(
            [self.zs, self.pis, self.rnn_mask1],
            feed_dict={self.x: x_, self.mb_size: np.shape(x_)[0], self.keep_prob: 1.0},
        )

    def predict_zbars_and_pis_m2(self, x_):
        return self.sess.run(
            [self.z_bars, self.pis, self.rnn_mask2],
            feed_dict={self.x: x_, self.mb_size: np.shape(x_)[0], self.keep_prob: 1.0},
        )
