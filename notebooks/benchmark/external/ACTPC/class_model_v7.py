...
class DeepTPC_ICLR:
        ...
        with tf.variable_scope(self.name):
            ...
            def loop_fn(time, cell_output, cell_state, loop_state):
                ...
                if cell_output is None:    # time == 0
                    ...
                    tmp_y = predictor(tmp_z, self.y_dim, self.y_type, self.num_layers_g, self.h_dim_g,
                                      self.fc_activate_fn)
                ...
                next_input = tf.cond(finished, lambda: tf.zeros([self.mb_size, self.x_dim], dtype=tf.float32),
            ...
            inputs_ta = tf.TensorArray(
                dtype=tf.float32, size=self.max_length).unstack(
                    _transpose_batch_time(inputs), name='rnn_input')

            cell = utils.create_rnn_cell(self.h_dim_f, self.num_layers_f, self.keep_prob, self.rnn_type,
            ...
            ### SAMPLING PROCESS
            s_dist = tf.distributions.Categorical(probs=tf.reshape(self.pis,
                                                                   [-1, self.K]))    #define the categorical dist.
            ...
            with tf.variable_scope('rnn', reuse=True):
                y_bars = predictor(z_bars, self.y_dim, self.y_type, self.num_layers_g, self.h_dim_g,
                                   self.fc_activate_fn)
            ...
            mean_pis = tf.reduce_sum(
                tf.reduce_sum(tmp_pis, axis=1), axis=0) / tf.reduce_sum(
            ...
            encoder_vars = [
                vars_ for vars_ in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
                ...
                for j in range(i + 1, self.K):
                    self.LOSS_Ey += -loss_1(
                        Ey[i, :], Ey[j, :], y_type_=self.y_type) / (
                            (self.K) * (self.K - 1))    # negative because we want to increase this;
            ...
            with tf.variable_scope('rnn', reuse=True):
                self.yy = predictor(self.zz, self.y_dim, self.y_type, self.num_layers_g, self.h_dim_g,
                                    self.fc_activate_fn)    #to check the predictor output given z
    ...
    def train_critic(self, x_, y_, beta_, lr_train, k_prob):
        return self.sess.run(
            [self.solver_L1_critic, self.LOSS_1, self.LOSS_2],
            feed_dict={
                self.x: x_,
                self.y: y_,
                self.beta: beta_,
    ...
    def train_actor(self, x_, y_, alpha_, gamma_, lr_train, k_prob):
        return self.sess.run(
            [self.solver_L1_actor, self.LOSS_1, self.LOSS_3],
            feed_dict={
                self.x: x_,
                self.y: y_,
                self.alpha: alpha_,
    ...
    def train_embedding(self, x_, y_, delta_, lr_train, k_prob):
        return self.sess.run(
            [self.solver_E, self.LOSS_1, self.LOSS_Ey],
            feed_dict={
                self.x: x_,
                self.y: y_,
                self.mb_size: np.shape(x_)[0],
