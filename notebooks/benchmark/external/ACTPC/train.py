...
if __name__ == '__main__':
    ...
    for out_itr in range(OUT_ITERATION):
        ...
        for itr in range(ITERATION):
            ...
            if (itr + 1) % check_step == 0:
                ...
                print(
                    "ITR {:04d}: loss_L2={:.3f}  loss_L1_c={:.3f}  loss_L1_a={:.3f}  loss_L1_E={:.3f}  loss_Ey={:.3f}  loss_L3_a={:.3f}"
                    .format(
                        itr + 1,
                        avg_loss_c_L2,
                        avg_loss_c_L1,
    ...
    pd.DataFrame(
        RESULT_NMI, columns=['NMI'], index=['itr' + str(out_itr) for out_itr in range(OUT_ITERATION)
                                           ]).to_csv('./{}/proposed/K{}/'.format(data_mode, K) + 'results_nmi.csv')

    pd.DataFrame(
        RESULT_RI, columns=['RI'], index=['itr' + str(out_itr) for out_itr in range(OUT_ITERATION)
                                         ]).to_csv('./{}/proposed/K{}/'.format(data_mode, K) + 'results_ri.csv')
