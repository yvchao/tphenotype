...
data_mode = 'CF_comorbidity_select' # 'ADNI'  #{'CF', 'CF_comorbidity', 'CF_comorbidity_select'}
...
pd.DataFrame(RESULT_NMI, 
             columns=['NMI'], 
             index=['itr'+str(out_itr) for out_itr in range(OUT_ITERATION)]).to_csv('./{}/proposed/K{}/'.format(data_mode, K) + 'results_nmi.csv')

pd.DataFrame(RESULT_RI, 
             columns=['RI'], 
             index=['itr'+str(out_itr) for out_itr in range(OUT_ITERATION)]).to_csv('./{}/proposed/K{}/'.format(data_mode, K) + 'results_ri.csv')

pd.DataFrame(RESULT_PURITY, 
             columns=['PURITY'], 
             index=['itr'+str(out_itr) for out_itr in range(OUT_ITERATION)]).to_csv('./{}/proposed/K{}/'.format(data_mode, K) + 'results_purity.csv')
