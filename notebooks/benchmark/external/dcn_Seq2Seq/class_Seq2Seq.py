...
#             inputs = tf.concat([self.x,self.y_onehot], axis=2, name='inputs') #delta + feature + label    
            ...
            enc_cell      = utils.create_rnn_cell(self.h_dim_f, self.num_layers_f, self.keep_prob, self.rnn_type, activation_fn=self.rnn_activate_fn)
            dec_cell      = utils.create_rnn_cell(self.h_dim_f, self.num_layers_f, self.keep_prob, self.rnn_type, activation_fn=self.rnn_activate_fn)
            

            with tf.variable_scope('Encoder'):
                initial_state = enc_cell.zero_state(self.mb_size, tf.float32)
                
                encoder_outputs, last_enc_state = tf.nn.dynamic_rnn(enc_cell,
                                                                    inputs=inputs,
            ...
            with tf.variable_scope('Decoder'):
                dummy_zero_input = tf.zeros(shape=[self.mb_size,
                                                   self.x_dim + self.y_dim], dtype=tf.float32,
                                            name='dummy_zero_input')
                ...
                def get_logits(tmp_h, reuse=tf.AUTO_REUSE):
                    with tf.variable_scope('Logit_Function', reuse=reuse):
                        for e in range(self.num_layers_g):
                            if e == 0:
                                h_delta = tmp_h
                            h_delta = tf.contrib.layers.fully_connected(inputs=h_delta, num_outputs=self.h_dim_g, activation_fn=self.fc_activate_fn)
                        h_delta    = tf.contrib.layers.fully_connected(inputs=h_delta, num_outputs=1, activation_fn=tf.nn.softplus)

                        for e in range(self.num_layers_g):
                            if e == 0:
                                h_x = tmp_h
                            h_x = tf.contrib.layers.fully_connected(inputs=h_x, num_outputs=self.h_dim_g, activation_fn=self.fc_activate_fn)
                        h_x        = tf.contrib.layers.fully_connected(inputs=h_x, num_outputs=self.x_dim-1, activation_fn=None)        

                        for e in range(self.num_layers_g):
                            if e == 0:
                                h_y = tmp_h
                            h_y = tf.contrib.layers.fully_connected(inputs=h_y, num_outputs=self.h_dim_g, activation_fn=self.fc_activate_fn)
                        h_y        = tf.contrib.layers.fully_connected(inputs=h_y, num_outputs=self.y_dim, activation_fn=tf.nn.softmax)
...
#             self.inputs_y       = tf.reshape(inputs[:, :, self.x_dim:], [-1, self.max_length, self.y_dim])                              
            ...
            loss_delta    = tf.reduce_mean(tf.reduce_sum(self.rnn_mask2*tf.reduce_sum((self.inputs_delta - self.outputs_delta)**2, axis=2), axis=1))
            loss_x        = tf.reduce_mean(tf.reduce_sum(self.rnn_mask2*tf.reduce_sum((self.inputs_x - self.outputs_x)**2, axis=2), axis=1))
            loss_y        = tf.reduce_mean(tf.reduce_sum(self.rnn_mask2*tf.reduce_sum(- self.y_onehot*log(self.outputs_y), axis=2), axis=1))
    ...
    def train_ae(self, x_, y_onehot_, a_delta, lr_train, k_prob):
        return self.sess.run([self.solver_AE, self.LOSS_AE],
                             feed_dict={self.x: x_, self.y_onehot: y_onehot_,
    ...
    def train_total(self, x_, y_onehot_, s_, E_, K_, a_delta, a, lr_train, k_prob):
        return self.sess.run([self.solver_TOTAL, self.LOSS_TOTAL, self.LOSS_AE, self.LOSS_CLU],
                             feed_dict={self.x: x_, self.y_onehot: y_onehot_, self.s: s_, self.E: E_, self.K: K_,
                                        self.mb_size:np.shape(x_)[0], self.a_delta: a_delta, self.a:a,
