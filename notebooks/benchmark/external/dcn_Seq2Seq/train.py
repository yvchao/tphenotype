...
if __name__ == '__main__':
    ...
    pd.DataFrame(RESULT_NMI, 
                 columns=['NMI'], 
                 index=['itr'+str(out_itr) for out_itr in range(OUT_ITERATION)]).to_csv('./{}/dcn_S2S/K{}/'.format(data_mode, K) + 'results_nmi.csv')

    pd.DataFrame(RESULT_RI, 
                 columns=['RI'], 
                 index=['itr'+str(out_itr) for out_itr in range(OUT_ITERATION)]).to_csv('./{}/dcn_S2S/K{}/'.format(data_mode, K) + 'results_ri.csv')

    pd.DataFrame(RESULT_PURITY, 
                 columns=['PURITY'], 
                 index=['itr'+str(out_itr) for out_itr in range(OUT_ITERATION)]).to_csv('./{}/dcn_S2S/K{}/'.format(data_mode, K) + 'results_purity.csv')
